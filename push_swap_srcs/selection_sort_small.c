...
/*   Updated: 2022/06/28 01:03:51 by nvasilev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_swap_optimizer(t_pos *stack_a, t_list **current, size_t index)
{
	...
	if (index == 0 && stack_a->size >= 3 && !is_sorted(stack_a))
	{
...
void	selection_sort_small(t_pos *stack_a, t_pos *stack_b)
{
	...
	while (current)
	{
		check_swap_optimizer(stack_a, &current, i);
		if (current->data == min)
		{
